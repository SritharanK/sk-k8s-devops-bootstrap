...
def serviceName = "surge-plugin"
def dockerRepo = "${dockerRegistry}/${serviceName}"

def serviceGitRepoUrl = env.SERVICE_GIT_REPO ?: "git@github.com:YOUR_ORG/surge-wp-plugin.git"
...
def gitUserCredsId = env.JENKINS_GIT_USER_CREDS_ID ?: error("JENKINS_GIT_USER_CREDS_ID not set in Jenkins global env")
def argocdCredsId = env.ARGOCD_CREDS_ID ?: error("ARGOCD_CREDS_ID not set in Jenkins global env")
    ...
    stages {
        ...
        stage('Update image version') {
            steps {
                script {
                    sshagent(credentials: [gitCredsId]) {
                        withCredentials([usernamePassword(credentialsId: gitUserCredsId, passwordVariable: 'GIT_PASS', usernameVariable: 'GIT_USER')]) {
...
def argocdFastExecutor (argocdServer, svc_name) {
    withCredentials([usernamePassword(credentialsId: argocdCredsId, usernameVariable: 'argoUser', passwordVariable: 'argoPasswd')]) {
        sh """
